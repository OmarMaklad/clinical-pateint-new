...
class BookDoctorController extends GetxController {
  ...
  Future getTimes({DateTime? date}) async {
    try {
      ...
      // /// Filter out times that are in the past or present
      // final now = DateTime.now();
      // times = times.where((time) {
      //   final _localDateTime = DateTime.parse(time.elementAt(0)).toLocal();
      //   return _localDateTime.isAfter(now);
      // }).toList();
      //
      // times.sort((e1, e2) {
      //   final _localDateTime1 = DateTime.parse(e1.elementAt(0)).toLocal();
  ...
  bool isCheckedPatient(Patient patient) {
    return (appointment.value.patient!.id) == patient.id;
  }
